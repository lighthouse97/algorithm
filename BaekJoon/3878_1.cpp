...
{
	int count = 0, size = ch.size(), j;
	double poc_x;
	for (int i = 0; i < size; i++) {
		j = (i + 1) % size;
		if ((p.y < ch[j].y) != (p.y < ch[i].y)) { // y 좌표로 p가 꼭지점 사이에 있는 경우
			poc_x = (ch[j].x - ch[i].x) * (p.y - ch[i].y) / (ch[j].y - ch[i].y) + ch[i].x; // p와 수평한 선과 변과의 접점의 x좌표를 구한다.
			if (p.x < poc_x) count++;
		}
	}
	return (count % 2) > 0; // 홀수면 p가 안, 짝수면 p가 밖에 있다.
}
